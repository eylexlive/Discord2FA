...
public class Discord2FACommand implements CommandExecutor {
    ...
    public Discord2FACommand() {
        ...
        this.mainMessage = new String[]{"§6§lDiscord2FA running on the server. §f§lVersion: §6§lv"+this.plugin.getDescription().getVersion(), "§fMade by:§6§l EylexLive"};
    }
    @Override
            ...
            if (args.length == 0) {
                ...
                if (args[0].equalsIgnoreCase("verifyList")) {
                    if (this.plugin.isMySQLEnabled()) {
                        sender.sendMessage("§cThis mode is turned off while mysql is on right now.");
                        return false;
                    }
                    ...
                    sender.sendMessage(Utils.translate(this.plugin.getConfig().getString(prefix+"reload-success")));
                }
                ...
                if (args[0].equalsIgnoreCase("removeFromCheck")) {
                    ...
                    this.plugin.getNMS().getSitInterface().unsitPlayer(bukkitPlayer);
